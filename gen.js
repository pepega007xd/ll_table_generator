...
function grammarChanged() {
    ...
    for (var rule of $element('grammar').value.split('\n')) {
        ...
        if (split_rule.length == 2) {
            let lhs_num = split_rule[0].trim().split(' ');
            let lhs = lhs_num[1].trim();
            let num = lhs_num[0].trim();
...
function displayTables() {
    ...
    for (var terminal of grammar.terminals) {
        ...
        $element('llTableHead').innerHTML += "<th style=\"writing-mode: vertical-lr\">"
            + terminal + "</th>";
    }
    ...
    for (var nonterminal of grammar.nonterminals) {
        ...
        var s = "<tr>";
        s += "<tr>";
        s += "<td nowrap=\"nowrap\" style=\"background-color: #e0e0e0;\">" + nonterminal + "</td>";
